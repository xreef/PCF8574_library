...
/*
 * PCF8574 Ultrasonic Simple Example
 *
 * This example demonstrates basic distance measurement using an HC-SR04
 * ultrasonic sensor connected to a PCF8574 I2C expander.
 *
 * Based on NewPing library's basic example, adapted for PCF8574.
 *
 * IMPORTANT NOTES:
 * - Reading via I2C has higher latency than direct GPIO
 * - Use polling methods for better I2C efficiency
 * - Precision is lower than direct connection
 * - Best for non-critical distance measurements
 *
 * Hardware Connections:
 * - PCF8574 P1 -> HC-SR04 TRIG
 * - PCF8574 P0 -> HC-SR04 ECHO
...
void setup() {
  ...
  Serial.println(F("\n=== PCF8574 Ultrasonic Simple Example ==="));
  Serial.println(F("Based on NewPing library example"));
...
void loop() {
  ...
  // Send ping, get distance in centimeters using polling method for efficiency
  unsigned int distance = pcf.ping_cm_poll(TRIGGER_PIN, ECHO_PIN, MAX_DISTANCE, 100);

  Serial.print(F("Ping: "));
  Serial.print(distance); // Send distance value to serial monitor
  Serial.println(F(" cm"));

  // Alternative: Get distance in inches
  // unsigned int distanceInches = pcf.ping_in_poll(TRIGGER_PIN, ECHO_PIN, MAX_DISTANCE, 100);
  // Serial.print(F("Ping: "));
